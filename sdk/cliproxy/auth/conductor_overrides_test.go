...
func TestManager_Update_DoesNotCarryStaleModelStatesIntoRefreshedAuth(t *testing.T) {
	m := NewManager(nil, nil, nil)
	...
	if _, err := m.Register(context.Background(), original); err != nil {
	...
	refreshed := &Auth{
		ID:       "auth-1",
		Provider: "claude",
		Metadata: map[string]any{"email": "fresh@example.com"},
	}
	...
	if len(updated.ModelStates) != 0 {
		t.Fatalf("expected refreshed auth to drop stale model states, got %+v", updated.ModelStates)
	}
	if updated.Unavailable {
		t.Fatalf("expected refreshed auth to be available")
	}
	if !updated.NextRetryAfter.IsZero() {
		t.Fatalf("expected refreshed auth NextRetryAfter to be zero, got %v", updated.NextRetryAfter)
	}
}
